...
glm.result <- glm(지각여부~., family='binomial',
                  data=emp)
...
bi_iris <- subset(iris, Species=='versicolor' |
                  Species=='virginica')
str(bi_iris)
...
# glm 함수로 로지스틱 회귀 적용
glm.result <- glm(Species~., family='binomial',
                  data=bi_iris)
...
bi_iris <- subset(iris, Species=='setosa' |
                    Species=='virginica')
head(bi_iris)
str(bi_iris)
...
glm.result <- glm(Species~Sepal.Width+Sepal.Length, family='binomial',
                  data=bi_iris)
...
train.idx <- createDataPartition(
  iris$Species, p=0.7, list=F)
